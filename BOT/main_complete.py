#!/usr/bin/env python3
"""
Bot Konversi Kontak Telegram - Versi Lengkap
...
def txt_to_vcf_conversion(file_path, base_name, contact_name, contacts_per_file, start_number):
    """Convert TXT to VCF files"""
    try:
        ...
        for number in numbers:
            if number.strip():
                cleaned = clean_phone_number(number.strip())
                if cleaned:
                    cleaned_numbers.append(cleaned)
        
        # Create VCF files
        vcf_files = []
        total_contacts = len(cleaned_numbers)
        
        for i in range(0, total_contacts, contacts_per_file):
            batch_numbers = cleaned_numbers[i:i + contacts_per_file]
            file_number = start_number + (i // contacts_per_file)
            
            vcf_content = ""
            for j, number in enumerate(batch_numbers):
                contact_num = i + j + 1
                vcf_content += f"BEGIN:VCARD\n"
                vcf_content += f"VERSION:3.0\n"
                vcf_content += f"FN:{contact_name} {contact_num}\n"
                vcf_content += f"TEL;TYPE=CELL:{number}\n"
                vcf_content += f"END:VCARD\n\n"
            
            vcf_filename = f"{base_name}_{file_number:03d}.vcf"
            vcf_path = os.path.join(TEMP_FOLDER, vcf_filename)
        ...
        return vcf_files, total_contacts
        
    except Exception as e:
        logger.error(f"Error in txt_to_vcf_conversion: {e}")
        return None, 0

def split_txt_file(file_path, lines_per_file):
    """Split TXT file"""
    try:
        with open(file_path, 'r', encoding='utf-8') as f:
            lines = f.readlines()
        ...
        for i in range(0, total_lines, lines_per_file):
            ...
            file_number = (i // lines_per_file) + 1
            
            split_filename = f"split_{file_number:03d}.txt"
            split_path = os.path.join(TEMP_FOLDER, split_filename)
        ...
        logger.error(f"Error in split_txt_file: {e}")
        return None, 0

def vcf_to_txt_conversion(file_path):
    ...
    try:
        ...
        # Save to TXT file
        output_filename = "extracted_numbers.txt"
        ...
        return output_path, len(cleaned_numbers)
        
    except Exception as e:
        logger.error(f"Error in vcf_to_txt_conversion: {e}")
        return None, 0

def xls_to_vcf_conversion(file_path):
    """Convert XLS/XLSX to VCF"""
    try:
        # Read Excel file
        ...
        numbers = df.iloc[:, 1].astype(str).tolist()
        ...
        for name, number in zip(names, numbers):
            if pd.isna(name) or pd.isna(number):
                continue
            
            cleaned_number = clean_phone_number(str(number))
            if cleaned_number:
                vcf_content += f"BEGIN:VCARD\n"
                vcf_content += f"VERSION:3.0\n"
                vcf_content += f"FN:{name.strip()}\n"
                vcf_content += f"TEL;TYPE=CELL:{cleaned_number}\n"
                vcf_content += f"END:VCARD\n\n"
                valid_contacts += 1
        
        # Save VCF file
        output_filename = "converted_contacts.vcf"
        output_path = os.path.join(TEMP_FOLDER, output_filename)
        
        with open(output_path, 'w', encoding='utf-8') as f:
            f.write(vcf_content)
        
        return output_path, valid_contacts
        
    except Exception as e:
        logger.error(f"Error in xls_to_vcf_conversion: {e}")
        return None, 0
...
def handle_file(message):
    ...
    try:
        ...
        if file_extension == '.txt':
            bot.reply_to(
                message,
                f"📁 File TXT diterima: {file_name}\n\n"
                f"Silakan pilih jenis konversi yang diinginkan:",
                reply_markup=conversion_menu()
            )
        elif file_extension == '.vcf':
            bot.reply_to(
                message,
                f"📁 File VCF diterima: {file_name}\n\n"
                f"Bot akan mengekstrak nomor telepon dari file ini.",
                reply_markup=back_button('back_to_features')
            )
            
            # Process VCF to TXT
            output_path, count = vcf_to_txt_conversion(file_path)
            if output_path and count > 0:
                with open(output_path, 'rb') as f:
                    bot.send_document(
                        message.chat.id,
                        f,
                        caption=f"✅ Berhasil mengekstrak {count} nomor telepon dari file VCF!"
                    )
            else:
                bot.reply_to(message, "❌ Gagal memproses file VCF.")
                
        elif file_extension in ['.xls', '.xlsx']:
            bot.reply_to(
                message,
                f"📁 File Excel diterima: {file_name}\n\n"
                f"Bot akan mengkonversi ke format VCF.",
                reply_markup=back_button('back_to_features')
            )
            
            # Process XLS to VCF
            output_path, count = xls_to_vcf_conversion(file_path)
            if output_path and count > 0:
                with open(output_path, 'rb') as f:
                    bot.send_document(
                        message.chat.id,
                        f,
                        caption=f"✅ Berhasil mengkonversi {count} kontak ke format VCF!"
                    )
            else:
                bot.reply_to(message, "❌ Gagal memproses file Excel.")
        else:
            bot.reply_to(
                message,
                f"❌ Format file tidak didukung: {file_extension}\n\n"
                f"Format yang didukung: .txt, .vcf, .xls, .xlsx",
                reply_markup=back_button('back_to_features')
            )
